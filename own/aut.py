...
'''
Encoding scheme:
Header: <r_skip> <g_skip> <b_skip> <a_skip> <r_pred> <g_pred> <b_pred> <a_pred> <r_bits> <g_bits> <b_bits> <a_bits> <yuva_colorspace?>
Data Line: (rgba) (rgba) (rgba) ... each 14 bits wide for a total of 448 bits
Total: 64 bit header + 448 bit data line to get a 64 byte cache line
'''
def own_compression(image, height, width):
    ...
    def compress_4x8(blk):
        ...
        # Compute header for YUVA 
        blk_yuva = np.zeros_like(blk, dtype=np.uint16)
        for i in range(4):
            for j in range(8): 
                yuva_pixel = rgba_to_yuva(blk[:, i, j])
                for c in range(4):
                    blk_yuva[c, i, j] = yuva_pixel[c]
        print(f"{blk}\n=============\n{blk_yuva}\n---------------\n")
        mins_yuva = np.zeros((blk_yuva.shape[0], ), dtype=np.int64)
        for i in range(mins_yuva.shape[0]): mins_yuva[i] = np.min(blk_yuva[i,:,:])
        res_yuva = blk_yuva.copy()
        for i in range(mins.shape[0]): res_yuva[i] -= int(mins_yuva[i])
        print(f"RES---\n{res}\n=============\n{res_yuva}\n---------------\n")
        max_res_yuva = np.zeros(mins_yuva.shape)
        for i in range(max_res_yuva.shape[0]): max_res_yuva[i] = int(np.max(res_yuva[i]))

        compression_bits_required = np.zeros(mins.shape)
        compression_bits_required_yuva = np.zeros(mins.shape)
        for i in range(compression_bits_required.shape[0]):
            if max_res_yuva[i] == 0:
                compression_bits_required_yuva[i] = 0
            else:
                log_val = np.log2(max_res_yuva[i])
                compression_bits_required_yuva[i] = math.ceil(log_val) + (log_val == math.floor(log_val))
            if max_res[i] == 0:
        ...
        print(f"RGBA {np.sum(compression_bits_required)} YUVA: {np.sum(compression_bits_required_yuva)}")
...
igpu = Aut(dataset, own_compression, intel_iGPU_decompress, intel_iGPU_random_access, name="intel iGPU")
