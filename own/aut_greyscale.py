...
'''
Encoding scheme:
Header: <r_skip> <g_skip> <b_skip> <a_skip> <r_pred> <g_pred> <b_pred> <a_pred> <r_bits> <g_bits> <b_bits> <a_bits> <greyscale?> 
...
def intel_iGPU_compress(image, height, width):
    ...
    def compress_4x8(blk):
        ...
        for i in range(4):
            for j in range(8):
                if(blk[0, i, j] != blk[1, i, j] or blk[0, i, j] != blk[2, i, j]):
                    greyscale = False 
                    break 
        
        # Achieve saving 
        if(greyscale):
            res[1, :, :] = np.zeros_like(res[0, :, :])
            res[2, :, :] = np.zeros_like(res[0, :, :])
