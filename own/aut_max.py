...
'''
Encoding scheme:
Header: <r_skip> <g_skip> <b_skip> <a_skip> <r_pred> <g_pred> <b_pred> <a_pred> <r_bits> <g_bits> <b_bits> <a_bits> <residual direction>
...
def intel_iGPU_compress(image, height, width):
    ...
    def compress_4x8(blk):
        ...
        # Calculate for opp
        maxs = np.zeros((blk.shape[0], ), dtype=np.int64)
        for i in range(maxs.shape[0]): maxs[i] = np.max(blk[i,:,:])
        res_opp = blk.copy()
        for i in range(maxs.shape[0]): res_opp[i] = (int(maxs[i]) - res_opp[i])
        max_res_opp = np.zeros(maxs.shape)
        for i in range(max_res_opp.shape[0]): max_res_opp[i] = int(np.max(res_opp[i]))


        compression_bits_required = np.zeros(mins.shape)
        compression_bits_required_maxs = np.zeros(maxs.shape)
        for i in range(compression_bits_required.shape[0]):
            if max_res_opp[i] == 0:
                compression_bits_required_maxs[i] = 0
            else:
                log_val = np.log2(max_res_opp[i])
                compression_bits_required_maxs[i] = math.ceil(log_val) + (log_val == math.floor(log_val))
            if max_res[i] == 0:
        ...
        if(np.sum(compression_bits_required) != np.sum(compression_bits_required_maxs)):
            print(f"min: {np.sum(compression_bits_required)} max: {np.sum(compression_bits_required_maxs)}")
