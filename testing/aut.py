...
def load_test_image(path):
    ...
    test_image = np.moveaxis(np.array(image), -1, 0)
    
    return test_image

def load_test_dataset(base_path, exts):
    # *.1.* - specify SIPI 512x512
    sample_files = itertools.chain(*[glob.glob(os.path.join(base_path, ext)) for ext in exts])

    dataset = np.array([load_test_image(sample_file) for sample_file in sample_files])

    return dataset

dataset_params = [('../samples/aerials/', ['*.1.*.tiff']), ('../samples/blur/', ['*.1.*.tiff_blur.png'])]

for base_path, exts in dataset_params:
    dataset = load_test_dataset(base_path, exts)
